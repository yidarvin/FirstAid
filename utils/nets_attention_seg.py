...
def Le_Net(layer, is_training, class_num, batch_size, keep_prob=1.0, name="Le_Net"):
    ...
    # Segmentation
    seg = deconv2d_w_bias(layer, 4, class_num, batch_size, name=name+"_deconv")
    # Attention layer
    attention = tf.split(tf.nn.softmax(seg), class_num, axis=3)
    attention = 1 - attention[0]
    attention = tf.nn.avg_pool(attention, ksize=[1,4,4,1], strides=[1,4,4,1], padding='VALID')
    layer *= attention
    # Collecting Attention and layer.
    visualization = layer
    visualization = tf.concat([attention, visualization], 3)
    # Dense Layers
    ...
    return layer, seg, visualization
...
def Alex_Net(layer, is_training, class_num, batch_size, keep_prob=1.0, name="Alex_Net"):
    ...
    layer = general_conv(layer, is_training, architecture_conv, name=name)
    # Segmentation
    seg = deconv2d_w_bias(layer, 32, class_num, batch_size, name=name+"_deconv")
    # Attention layer
    attention = tf.split(tf.nn.softmax(seg), class_num, axis=3)
    attention = 1 - attention[0]
    attention = tf.nn.avg_pool(attention, ksize=[1,32,32,1], strides=[1,32,32,1], padding='VALID')
    layer *= attention
    # Collecting Attention and layer.
    visualization = layer
    visualization = tf.concat([attention, visualization], 3)
...
def inceptionv1_module(layer, is_training, kSize=[16,16,16,16,16,16], name="inceptionv1_module"):
    ...
    #layer_5x5b = conv2d_bn_relu(layer_5x5a, is_training, 3, kSize[4], name=(name+"_5x5b"))
...
def GoogLe_Net(X, is_training, class_num, batch_size, keep_prob=1.0, name="GoogLe_Net"):
    ...
    layer = tf.nn.avg_pool(layer, ksize=[1,size_pool,size_pool,1], strides=[1,1,1,1], padding='VALID')# * size_pool * size_pool
