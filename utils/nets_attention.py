...
def GoogLe_Net(X, is_training, class_num, batch_size, keep_prob=1.0, name="GoogLe_Net"):
    ...
    #                     [11,256],[0,2],
    #                     [3,384],[3,384],[3,256],[0,2]]
    #attention = general_conv(X, is_training, architecture_conv, name=name+"_alexBranch")
    attention = conv2d_wo_bias(layer, 1, 1, stride=1, name=name+"_attention")
    attention = tf.sigmoid(attention)# * tf.nn.avg_pool(X, ksize=[1,32,32,1], strides=[1,32,32,1], padding='VALID')
    layer *= attention# / (tf.reduce_sum(attention) + 0.000001)
    # Collecting Attention and layer.
    visualization = layer
    visualization = tf.concat([attention, visualization], 3)
    # Output
